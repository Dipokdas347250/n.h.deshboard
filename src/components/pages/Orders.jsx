...
export default function Orders() {
  ...
  return <section className="min-h-screen bg-[#064e3b]/90 p-6 text-white"><h1 className="mb-6 text-2xl font-bold">Orders</h1>{error && <p className="mb-4 rounded bg-red-500/20 p-3 text-red-200">{error}</p>}<div className="overflow-x-auto rounded-xl bg-white/10"><table className="w-full text-left"><thead><tr className="border-b border-white/20 text-gray-300"><th className="p-4">Customer</th><th className="p-4">Delivery Address</th><th className="p-4">Amount</th><th className="p-4">Payment</th><th className="p-4">Delivery</th><th className="p-4">Action</th></tr></thead><tbody>{orders.map((order) => { const customer = order.customer || order.shipping || {}; return <tr key={order._id} className="border-b border-white/10"><td className="p-4">{customer.name || order.user?.fullname || "Guest"}<span className="block text-xs text-gray-400">{customer.email || order.user?.email}</span><span className="block text-xs text-gray-400">{customer.phone}</span></td><td className="p-4 text-sm">{customer.address}<span className="block text-gray-400">{[customer.city, customer.district, customer.postcode].filter(Boolean).join(", ")}</span></td><td className="p-4">৳{(order.totalprice || 0).toLocaleString()}</td><td className="p-4">{order.paymentStatus}</td><td className="p-4">{order.deliveryStatus}</td><td className="p-4"><select value={order.deliveryStatus} onChange={(event) => update(order._id, event.target.value)} className="rounded bg-white/10 p-2"><option value="pending">Pending</option><option value="confirm">Confirmed</option><option value="deliverd">Delivered</option><option value="cenceled">Cancelled</option></select></td></tr>; })}</tbody></table>{!orders.length && <p className="p-8 text-center text-gray-300">No orders found.</p>}</div></section>;
}
